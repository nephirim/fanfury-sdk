...
func TestMsgAggregateExchangeRatePrevote(t *testing.T) {
	...
	tests := []struct {
		...
		{[]byte("invalid hash"), exchangeRates, addrs[0], addrs[0], false, ErrInvalidHash.Error()},
...
func TestMsgAggregateExchangeRateVote(t *testing.T) {
	...
	msgInvalidHexString := fmt.Sprintf("salt must be a valid hex string: %s", ErrInvalidSaltFormat.Error())
	...
	msgInvalidExchangeRates := fmt.Sprintf("failed to parse exchange rates string cause: %s a: invalid coins",
		ErrInvalidExchangeRate.Error())
	...
	tests := []struct {
		...
		{addrs[0], addrs[0], "", exchangeRates, false, ErrInvalidSaltLength.Error()},
